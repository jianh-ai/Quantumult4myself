            ...
            try {
                ...
                for (let o of s.data.items) {
                    if (!o.hasOwnProperty("banner_item")) {
                        if (!o.hasOwnProperty("ad_info") && o.card_goto?.indexOf("ad") === -1 &&
        ...
        case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test($request.url):
            try {
                ...
                if (h.data?.tab) {
                    ...
                    f.push({ id: 40, name: "推荐", uri: "bilibili://pegasus/promo", tab_id: "推荐tab", pos: 2, default_selected: 1 });
                    ...
                    f.push({ id: 41, name: "热门", uri: "bilibili://pegasus/hottopic", tab_id: "hottopic", pos: 4 });
                    f.push({ id: 151, name: "影视", uri: "bilibili://pgc/cinema-tab", tab_id: "film", pos: 5 });
                    h.data.tab = f;
                }
                if (h.data.top) {
                    h.data.top = [{ id: 481, icon: "http://i0.hdslb.com/bfs/archive/d43047538e72c9ed8fd8e4e34415fbe3a4f632cb.png", name: "消息", uri: "bilibili://link/im_home", tab_id: "消息Top", pos: 1 }];
                }
                if (h.data.bottom) {
        ...
        case /^https?:\/\/api\.live\.bilibili\.com\/xlive\/app-room\/v1\/index\/getInfoByRoom/.test($request.url):
            try {
                ...
                if (x.data?.new_tab_info?.outer_list && x.data.new_tab_info.outer_list.length) {
                    x.data.new_tab_info.outer_list = x.data.new_tab_info.outer_list.filter(t => t.biz_id !== 33);
                }
